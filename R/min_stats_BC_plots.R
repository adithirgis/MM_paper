...
plot3
ggsave(here("Plots", "BC_CO2_point_All.jpg"), width = 30, height = 20, units = "cm")
...
ggsave(here("Plots", "UFPs_CO2_high_All.jpg"), width = 30, height = 20, units = "cm")

plot4 %+% subset(Final_all, Road_type %in% c("Arterial")) %+%
  annotate("text", x = 60, y = 10, label = "Arterial", size = 7)
ggsave(here("Plots", "UFPs_CO2_art_All.jpg"), width = 30, height = 20, units = "cm")

plot4 %+% subset(Final_all, Road_type %in% c("Residential")) %+%
  annotate("text", x = 60, y = 10, label = "Residential", size = 7)
ggsave(here("Plots", "UFPs_CO2_resi_All.jpg"), width = 30, height = 20, units = "cm")

plot41 <- ggplot(subset(Final_all, CO2_c != 0), aes(x = Speed, y = as.numeric(as.character(CPC/CO2_c)))) + 
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)),
  ...
  scale_fill_viridis(option = "plasma", limits = c(0, 200)) 
plot41
ggsave(here("Plots", "UFPs_CO2_all_All.jpg"), width = 30, height = 20, units = "cm")


plot5 <- ggplot(data = subset(Final_all, CO2_c != 0), aes(x = Speed, y = as.numeric(as.character(BC_NR_LC/CO2_c)))) + 
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)),
...
plot5 %+% annotate("text", x = 60, y = 0.01, label = "Highway", size = 7) %+% 
  subset(Final_all, Road_type %in% c("Highway")) 
ggsave(here("Plots", "BC_CO2_high_All.jpg"), width = 30, height = 20, units = "cm")

plot5 %+% subset(Final_all, Road_type %in% c("Arterial")) %+%
  annotate("text", x = 60, y = 0.01, label = "Arterial", size = 7)
ggsave(here("Plots", "BC_CO2_art_All.jpg"), width = 30, height = 20, units = "cm")

plot5 %+% subset(Final_all, Road_type %in% c("Residential")) %+%
  annotate("text", x = 60, y = 0.01, label = "Residential", size = 7)
ggsave(here("Plots", "BC_CO2_resi_All.jpg"), width = 30, height = 20, units = "cm")



plot51 <- ggplot(data = subset(Final_all, CO2_c != 0), aes(x = Speed, y = as.numeric(as.character(BC_NR_LC/CO2_c)))) + 
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x)),
  ...
  scale_x_continuous(limits = c(0, 72), breaks = c(0, 20, 40, 60))
plot51
ggsave(here("Plots", "BC_CO2_all_All.jpg"), width = 30, height = 20, units = "cm")
