...
Final_reference <- readOGR("D:/Dropbox/APMfull/MAL_CNG_Paper/Final_layers/Corrected_Final_MAL2_Layer.shp", 
...
Final_reference <- spTransform(Final_reference, CRS("+proj=utm +zone=43 ellps=WGS84"))
...
plo <- ggplot(df, aes(x = as.numeric(as.character(layer_N)), 
  ...
  ylab("NRMSE") + ggtitle("NRMSE in Data Only Analysis in Malleshwaram 2 for BC") + 
...
plo1 <- ggplot(df, aes(x = as.numeric(as.character(layer_N)), 
  ...
  ylab("R Squared") + ggtitle("Monte Carlo Analysis in Malleshwaram 2 for Ultrafines particle") + 
  theme(legend.text = element_text(size=18),
...
names(df) <- c("Layer", "Numeric Layer", "Y Mean Using Sum", "Y Mean using Mean",
               ...
               "Y min", "Y max")
myOutFile <- paste("D:/Dropbox/APMfull/Phase_II/MAL1/Analysis_BC_MAL1_Corrected.csv")
